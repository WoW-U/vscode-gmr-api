...
let wow_config = vscode.workspace.getConfiguration("gmrAPI");

const author = "amstaffix";
const extensionName = "gmr-lua-api";
...
vscode.workspace.onDidChangeConfiguration((event: vscode.ConfigurationChangeEvent) => {
	...
	if (event.affectsConfiguration("gmrAPI")) {
		wow_config = vscode.workspace.getConfiguration("gmrAPI");
		if (event.affectsConfiguration("gmrAPI.luals.configurationScope")) {
			configLuaLS(true);
		}
	}
});
...
export function isConfigured() {
	...
	return lib.find((value) => value.includes("gmr-api")) || lib.find((value) => value.includes("gmr-lua-api"));;
}